...
#### GUI
Download from [releases](https://github.com/RayBytes/ChatMock/releases) (macOS & Windows; deferred/transitional path)

#### Docker
See [DOCKER.md](DOCKER.md)

Status:
- Primary server/runtime implementation: Rust (Phase 7 live/runtime validation is green).
- The packaged CLI release path still uses the existing Python-based release flow.
- The Python CLI remains the explicit fallback path.
- The GUI path remains deferred/transitional.
- Rollback remains explicit: keep using the packaged Python CLI path for release installs, or disable `--responses-websocket-upstream-stateful` to return HTTP `/v1/responses` follow-ups to one-shot behavior.
...
The current packaged install path still follows `python-release-path-unchanged`. `chatmock login` and `chatmock serve` remain available as the explicit rollback/fallback path while packaging stays transitional.

<br>

## Usage

The examples below target the OpenAI-compatible server surface exposed by the Rust runtime above. The README-compatible `/v1/chat/completions` smoke test with `gpt-5.4` was revalidated live and returned `pong`.
...
All flags go after `cargo run -- serve` for the Rust runtime, or after `chatmock serve` for the packaged Python fallback CLI. These can also be set as environment variables.
...
- In transport-only mode, HTTP follow-up requests stay one-shot even if the client resends `previous_response_id`: ChatMock does not reuse an upstream websocket across requests and still does not reuse that marker automatically.
...
6. For a streaming follow-up with an invalid marker, confirm the current limitation: HTTP `200` SSE with the first event `type=error` and `status_code=400`, not a pre-commit HTTP `400`.
